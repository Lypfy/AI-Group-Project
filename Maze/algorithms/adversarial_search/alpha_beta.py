...
class AlphaBeta:
    ...
    def alphabeta(self, depth, alpha, beta, is_max, matrix):
        ...
        if is_max:
            ...
            for move in moves:
                new_matrix = self.apply_move([row[:] for row in matrix], move, 3)
                next_is_max = False if self.get_location(new_matrix, 4) else True
                val, _ = self.alphabeta(depth - 1, alpha, beta, next_is_max, new_matrix)
                ...
                if depth == self.max_depth and alpha != old_alpha:
                    dir_vn = self.translate_dir(move)
                    self.search_history.append(([row[:] for row in matrix], self.explored_count, 0, f"<font color='#00FFFF'>[Alpha-Beta]</font> Cập nhật <font color='#FFA500'>Alpha={round(alpha,1)}</font> (xét hướng {dir_vn})"))

                if beta <= alpha:
                    if depth == self.max_depth:
                        dir_vn = self.translate_dir(move)
                        self.search_history.append(([row[:] for row in matrix], self.explored_count, 0, f"<font color='#FF00FF'>[Cắt tỉa]</font> Cắt nhánh tại hướng {dir_vn} (Beta={round(beta,1)} &le; Alpha={round(alpha,1)})"))
                    break
            ...
            for move in moves:
                new_matrix = self.apply_move([row[:] for row in matrix], move, 4)
                val, _ = self.alphabeta(depth - 1, alpha, beta, True, new_matrix)
                ...
                if depth == self.max_depth and beta != old_beta:
                    dir_vn = self.translate_dir(move)
                    self.search_history.append(([row[:] for row in matrix], self.explored_count, 0, f"<font color='#00FFFF'>[Alpha-Beta]</font> Cập nhật <font color='#FFA500'>Beta={round(beta,1)}</font> (xét hướng {dir_vn})"))
