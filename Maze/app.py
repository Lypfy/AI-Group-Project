...
class MazeApp:
    ...
    def __init__(self, root):
        ...
        self.root.title(
            "Maze Solver AI - Multi-Levels & Algorithms"
        )
            ...
            for j in range(cols):
                ...
                if value == 1:
                    ...
                    if value == 3:
                        ...
                        color = "#585b70"  # Xám nhạt/sáng hơn nền nhiều để nổi bật dấu chân
                        text_char = "👣"
    ...
    def run_algorithm(self):
        ...
        self.path_count_label.config(
            text=f"{len(actions)} bước"
        )
        ...
        bbox = self.solution_canvas.bbox(
            self.solution_text
        )
        ...
        self.log(
            f"Thời gian: {thinking_time_ms:.2f} ms",
            "success"
        )

        self.log(
            f"Số bước: {len(actions)}",
            "success"
        )
    ...
    def animate_step(self):
        ...
        if self.step_idx < len(self.path):
            ...
            self.visited_label.config(text=f"Visited: {self.step_idx}")
            self.path_label.config(text=f"Path: {len(self.path)}")
            self.frontier_label.config(text=f"Frontier: {len(self.path) - self.step_idx}")
